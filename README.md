...
[![arduino-library-badge](https://www.ardu-badge.com/badge/AsyncESP32_ENC_Manager.svg?)](https://www.ardu-badge.com/AsyncESP32_ENC_Manager)
[![GitHub release](https://img.shields.io/github/release/khoih-prog/AsyncESP32_ENC_Manager.svg)](https://github.com/khoih-prog/AsyncESP32_ENC_Manager/releases)
[![GitHub](https://img.shields.io/github/license/mashape/apistatus.svg)](https://github.com/khoih-prog/AsyncESP32_ENC_Manager/blob/main/LICENSE)
[![contributions welcome](https://img.shields.io/badge/contributions-welcome-brightgreen.svg?style=flat)](#Contributing)
[![GitHub issues](https://img.shields.io/github/issues/khoih-prog/AsyncESP32_ENC_Manager.svg)](http://github.com/khoih-prog/AsyncESP32_ENC_Manager/issues)
...
* [Why do we need this AsyncESP32_ENC_Manager library](#why-do-we-need-this-async-AsyncESP32_ENC_Manager-library)
  * [Features](#features)
...
* [Debug Terminal Output Samples](#debug-terminal-output-samples)
  ...
  * [2. Async_ConfigOnSwichFS using LittleFS on ESP32_DEV with ESP32_ENC28J60](#2-Async_ConfigOnSwichFS-using-LittleFS-on-ESP32_DEV-with-ESP32_ENC28J60)
  * [3. Async_ESP32_FSWebServer_DRD using LittleFS on ESP32_DEV with ESP32_ENC28J60](#3-Async_ESP32_FSWebServer_DRD-using-LittleFS-on-ESP32_DEV-with-ESP32_ENC28J60)
* [Debug](#debug)
...
### Why do we need this Async [AsyncESP32_ENC_Manager library](https://github.com/khoih-prog/AsyncESP32_ENC_Manager)
...
## Prerequisites
 ...
 7. [`WebServer_ESP32_ENC library v1.5.1+`](https://github.com/khoih-prog/WebServer_ESP32_ENC) if necessary to use ESP32 boards using LwIP ENC28J60 Ethernet. To install, check [![arduino-library-badge](https://www.ardu-badge.com/badge/WebServer_ESP32_ENC.svg?)](https://www.ardu-badge.com/WebServer_ESP32_ENC)
...
The best and easiest way is to use `Arduino Library Manager`. Search for `AsyncESP32_ENC_Manager`, then select / install the latest version. You can also use this link [![arduino-library-badge](https://www.ardu-badge.com/badge/AsyncESP32_ENC_Manager.svg?)](https://www.ardu-badge.com/AsyncESP32_ENC_Manager) for more detailed instructions.

### Manual Install

1. Navigate to [AsyncESP32_ENC_Manager](https://github.com/khoih-prog/AsyncESP32_ENC_Manager) page.
2. Download the latest release `AsyncESP32_ENC_Manager-main.zip`.
3. Extract the zip file to `AsyncESP32_ENC_Manager-main` directory 
4. Copy the whole `AsyncESP32_ENC_Manager-main` folder to Arduino libraries' directory such as `~/Arduino/libraries/`.

### VS Code & PlatformIO:

1. Install [VS Code](https://code.visualstudio.com/)
2. Install [PlatformIO](https://platformio.org/platformio-ide)
3. Install [**AsyncESP32_ENC_Manager** library](https://registry.platformio.org/libraries/khoih-prog/AsyncESP32_ENC_Manager) by using [Library Manager](https://registry.platformio.org/libraries/khoih-prog/AsyncESP32_ENC_Manager/installation). Search for **AsyncESP32_ENC_Manager** in [Platform.io Author's Libraries](https://platformio.org/lib/search?query=author:%22Khoi%20Hoang%22)
...
Thanks to [Roshan](https://github.com/solroshan) to report the issue in [Error esp_littlefs.c 'utime_p'](https://github.com/khoih-prog/AsyncESP32_ENC_Manager/issues/28) 
...
```cpp
#include <AsyncESP32_ENC_Manager.hpp>               //https://github.com/khoih-prog/AsyncESP32_ENC_Manager
```

in many files. But be sure to use the following `#include <AsyncESP32_ENC_Manager.h>` **in just 1 `.h`, `.cpp` or `.ino` file**, which must **not be included in any other file**, to avoid `Multiple Definitions` Linker Error

```cpp
// To be included only in main(), .ino with setup() to avoid `Multiple Definitions` Linker Error
#include <AsyncESP32_ENC_Manager.h>          //https://github.com/khoih-prog/AsyncESP32_ENC_Manager
```

Check [Async_ConfigOnDoubleReset_Multi](examples/Async_ConfigOnDoubleReset_Multi) for an example how and where to do so.

Have a look at the discussion in [Different behaviour using the src_cpp or src_h lib #80](https://github.com/khoih-prog/AsyncESP32_ENC_Manager/discussions/80)
...
// Must be placed before #include <AsyncESP32_ENC_Manager.h>
...
///////////////////////////////////////////
/******************************************
   // Defined in AsyncESP32_ENC_Manager.hpp
  typedef struct
  {
...
These illustrating steps is based on the example [Async_ConfigOnSwitchFS](https://github.com/khoih-prog/AsyncESP32_ENC_Manager/tree/main/examples/Async_ConfigOnSwitchFS)
...
The example [Async_ConfigOnSwitchFS](https://github.com/khoih-prog/AsyncESP32_ENC_Manager/tree/main/examples/Async_ConfigOnSwitchFS) will open the CP whenever a SW press is detected in `loop()`. So the code to add `dynamic variables` will be there, just after the CP `AsyncESP32_ENC_Manager` class initialization to create `AsyncESP32_ENC_Manager` object.
...
Adding those `ESPAsync_EMParameter` objects created in Step 2 using the function `addParameter()` of object `AsyncESP32_ENC_Manager`
...
After users select `Save`, the CP `AsyncESP32_ENC_Manager` object will save the user input data into related `ESPAsync_EMParameter` objects.
...
<p align="center">
    <img src="https://github.com/khoih-prog/AsyncESP32_ENC_Manager/raw/main/Images/Main.png">
</p>

Select `Information` to enter the Info page where the board info will be shown (long page)

<p align="center">
    <img src="https://github.com/khoih-prog/AsyncESP32_ENC_Manager/raw/main/Images/Info.png">
</p>
...
<p align="center">
    <img src="https://github.com/khoih-prog/AsyncESP32_ENC_Manager/raw/main/Images/Configuration_Standard.png">
</p>
...
<p align="center">
    <img src="https://github.com/khoih-prog/AsyncESP32_ENC_Manager/raw/main/Images/Saved.png">
</p>
...
Many applications need configuration parameters like `MQTT host and port`, [Blynk](http://www.blynk.cc) or [emoncms](http://emoncms.org) tokens, etc. While it is possible to use [`AsyncESP32_ENC_Manager`](https://github.com/khoih-prog/AsyncESP32_ENC_Manager) to collect additional parameters, it is better to read these parameters from a web service once [`AsyncESP32_ENC_Manager`](https://github.com/khoih-prog/AsyncESP32_ENC_Manager) has been used to connect to the Internet.

To capture other parameters with [`AsyncESP32_ENC_Manager`](https://github.com/khoih-prog/AsyncESP32_ENC_Manager) is a little bit more complicated than all the other features. This requires adding custom HTML to your form. 

If you want to do it with [`AsyncESP32_ENC_Manager`](https://github.com/khoih-prog/AsyncESP32_ENC_Manager) see the example [Async_ConfigOnSwitchFS](examples/Async_ConfigOnSwitchFS)
...
https://github.com/khoih-prog/AsyncESP32_ENC_Manager/blob/d59df77734329c2097b5836508fa807bcde23124/examples/Async_ConfigOnSwitch/Async_ConfigOnSwitch.ino#L28-L924
...
This is terminal debug output when running [Async_ConfigOnDoubleReset](examples/Async_ConfigOnDoubleReset) on  **ESP32 + LwIP ENC28J60**. `Config Portal` was requested by DRD to input and save Credentials, such as Static IP address.

```
Starting Async_ConfigOnDoubleReset using LittleFS on ESP32_DEV with ESP32_ENC28J60
AsyncESP32_ENC_Manager v1.0.0
...
ETH Connected
ETH MAC: DE:AD:BE:EF:FE:0B, IPv4: 192.168.2.96
FULL_DUPLEX, 10Mbps
...
Starting configuration portal @ 192.168.2.96
[EM] _configPortalStart millis() = 2482
[EM] Config Portal IP address = 192.168.2.96
[EM] HTTP server started
[EM] startConfigPortal : Enter loop
[EM] handleRoot
[EM] request host IP = 192.168.2.96
[EM] request host IP = 192.168.2.96
[EM] Info
[EM] Info page sent
[EM] handleRoot
...
[EM] OK
Current IP = 192.168.2.96. Reset to take new IP = 192.168.2.232
...
Local Date/Time: Thu Dec  8 20:32:54 2022
Stop doubleResetDetecting
Saving config file...
Saving config file OK
Local Date/Time: Thu Dec  8 20:33:52 2022
```

---

#### 2. Async_ConfigOnSwichFS using LittleFS on ESP32_DEV with ESP32_ENC28J60

This is terminal debug output when running [Async_ConfigOnSwichFS](examples/Async_ConfigOnSwichFS) on  **ESP32 + LwIP ENC28J60**. `Config Portal` was requested to input and save Credentials.


```
Starting Async_ConfigOnSwichFS using LittleFS on ESP32_DEV with ESP32_ENC28J60
AsyncESP32_ENC28J60_Manager v1.0.0
[EM] Default SPI pinout:
[EM] MOSI: 23
[EM] MISO: 19
[EM] SCK: 18
...
Config file was successfully saved
[EM] freeing allocated params!
Local Date/Time: Thu Dec  8 20:36:52 2022
Local Date/Time: Thu Dec  8 20:37:52 2022
```

---

#### 3. Async_ESP32_FSWebServer_DRD using LittleFS on ESP32_DEV with ESP32_ENC28J60

This is terminal debug output when running [Async_ESP32_FSWebServer_DRD](examples/Async_ESP32_FSWebServer_DRD)  on  **ESP32 + LwIP ENC28J60**. `Config Portal` was requested by DRD (also using **LittleFS**) to input and save Credentials.


```cpp
Starting Async_ESP32_FSWebServer_DRD using LittleFS on ESP32_DEV with ESP32_ENC28J60
AsyncESP32_ENC28J60_Manager v1.0.0
ESP_DoubleResetDetector v1.3.2
...
Submit issues to: [AsyncESP32_ENC_Manager issues](https://github.com/khoih-prog/AsyncESP32_ENC_Manager/issues)
...
- The library is licensed under [MIT](https://github.com/khoih-prog/AsyncESP32_ENC_Manager/blob/main/LICENSE)
